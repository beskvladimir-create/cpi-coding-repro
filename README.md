...
| `src/aggregation_to_training.py` | End-to-end link between labeling and modeling: training labels replaced by simulated crowd votes, aggregated by majority, the reliability-weighted rule and Dawid-Skene, then used to train the classifiers, scored on clean test labels. Summary in `results/aggregation_to_training_summary.csv`; the large per-run file is regenerated by `make audit`. |
